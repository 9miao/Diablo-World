...
from core.character.Monster import Monster
from core.fight.battleSide import BattleSide
from core.Item import Item
from utils.dbopera import db_zhanyi
from applyInterface import dropout
...
class CharacterZhanYiComponent(Component):
    ...
    def initData(self):
        ...
        self.record = db_zhanyi.getCharacterZhangJieInfo(characterId)
        
    def getCurrentZJ(self):
        '''获取角色当前战役的信息'''
        if self.record:
            _currentZJ = sorted(self.record.keys())[-1]
            zjlist = sorted(db_zhanyi.ALL_ZHANGJIE_INFO.keys())
            index = zjlist.index(_currentZJ)+1
            if index<len(zjlist):
                _currentZJ = zjlist[index]
        else:
            _currentZJ = 1000
        return _currentZJ
    ...
    def _getZhanYiInfo(self,index):
        ...
        if index ==0:
            ...
            zid = index
        gropInfo = db_zhanyi.ALL_ZHANGJIE_GROP.get(zid)
        if not gropInfo:
            return {}
#            return {'result':False,'message':u'地图信息不存在'}
        ...
        for cityid in gropInfo:
            info = {}
            city = db_zhanyi.ALL_ZHANGJIE_INFO.get(cityid)
            cityId = city['id']
            info['mid'] = cityId
...
#        return {'result':False,'message':u'','data':info}
    ...
    def doZhangJie(self,zhangjieid):
        ...
        zhanjieInfo = db_zhanyi.ALL_ZHANGJIE_INFO.get(zhangjieid)
        if not zhanjieInfo:
        ...
        from core.fight.fight_new import Fight
        ...
        if data.battleResult == 1:#如果战斗胜利
            ...
            if zhangjieid  not in self.record.keys():
                ...
                db_zhanyi.InsertNewCharacterJQ(characterId,_reord)
            elif self.record[zhangjieid]['star']<battlestar:
                db_zhanyi.UpdateCharacterJQ(characterId, zhangjieid, battlestar)
                self.record[zhangjieid]=_reord
            zhanjieInfo = db_zhanyi.ALL_ZHANGJIE_INFO.get(zhangjieid)
            coinbound = zhanjieInfo.get('coin')
            expBound = zhanjieInfo.get('exp')
            dropid = zhanjieInfo.get('dropid')
            dropoutIitem = dropout.getDropByid(dropid)
            if dropoutIitem:
        ...
        self._owner.matrix.cleanMatrixSetting(result = data.battleResult,
                                              zyid=zhangjieid)
    ...
    def getCityInfo(self,mid):
        '''获取单个城镇的信息
        '''
        zhanjieInfo = db_zhanyi.ALL_ZHANGJIE_INFO.get(mid)
        info = {}
        info['mid'] = mid
